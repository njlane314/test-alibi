...
import { runScan, type ScanOptions } from './index.js';
...
function detectRefs(eventPath?: string): { base: string; head: string } {
  ...
  if (!base || !head) throw new Error('base and head refs are required outside pull_request events');
  return { base, head };
}

async function upsertComment(markdown: string): Promise<void> {
  const token = input('github-token');
  const issue = github.context.issue;
  if (!token || !issue.owner || !issue.repo || !issue.number) return;
...
async function main(): Promise<void> {
  try {
    ...
    const options: ScanOptions = {
      ...
      head: refs.head,
      cwd: process.cwd(),
      mode: (input('mode') as 'warn' | 'fail' | undefined) ?? 'warn'
    };
    ...
    if (eventPath) options.eventPath = eventPath;
    if (modelPath) options.modelPath = modelPath;
    if (since) options.since = since;
    if (coverage) options.coverage = coverage;

    const result = await runScan(options);
    const markdown = renderMarkdown(result);
    await writeStepSummary(markdown);
    if (boolInput('comment')) await upsertComment(markdown);
    core.setOutput('json', JSON.stringify(result));
    core.setOutput('markdown', markdown);
    core.setOutput('findings', String(result.summary.findings));
    if (exitCodeFor(result) !== 0) core.setFailed('Test Alibi found blocking findings.');
  } catch (error) {
